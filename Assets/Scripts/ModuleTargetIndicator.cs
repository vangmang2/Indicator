...
public static class ModuleTargetIndicator
{
    ...
    static Vector3 outOfBorderPos => new Vector3(Screen.width * indicatorBorder.x, Screen.height * indicatorBorder.y);

    /// <summary> 3d 좌표의 타겟을 2d UI 좌표계로 변환 해준다. 인디케이터 경계는 원형 경계로 처리한다. </summary>
    public static Vector2 IndicateTarget(this Camera uiCamera, Camera mainCamera, Transform target, out bool isOutOfBorder)
    {
        ...
        {
            targetUICameraPos.x *= Screen.width;
            targetUICameraPos.y *= Screen.height;
        }
